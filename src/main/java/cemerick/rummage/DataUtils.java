 ...
 *  
 *  Please see the "NOTE" in the Javadoc for this class, which describes its provenance.
 */
 ...
 *  -----------------------------
 *  
 *  NOTE: This class originated in the first AWS SimpleDB java client.
 *  Typica adopted it wholesale for use in their library.  We are only interested
 *  in its numerics-formatting methods, and don't want to deal with dependency
 *  conflicts between Typica and the official AWS API implementation for Java
 *  (and we prefer the latter).  Thus, this is r346 of this class, pulled from
 *  Typica's google code svn repo.  The original copyright notices are in the
 *  source file, above this note.
 * 
 */

package cemerick.rummage;
